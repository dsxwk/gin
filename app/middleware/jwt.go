...
func (s Jwt) Decode(jwtToken string) (map[string]interface{}, error) {
	token, err := jwt.Parse(jwtToken, func(token *jwt.Token) (interface{}, error) {
		...
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf(lang.T(s.Context.Get(), "middleware.jwt.unsupportedSignatureMethod", nil)+": %v", token.Header["alg"])
		}
	...
	if err != nil {
		return nil, fmt.Errorf(lang.T(s.Context.Get(), "middleware.jwt.TokenParseErr", nil)+": %v", err)
	}
	...
	return nil, fmt.Errorf(lang.T(s.Context.Get(), "middleware.jwt.InvalidToken", nil))
}
