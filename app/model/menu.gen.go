...
type Menu struct {
	...
	Children  []utils.TreeNode `gorm:"-" json:"children"`
...
func (m *Menu) GetChildren() *[]utils.TreeNode {
	return &m.Children
}

func (m *Menu) GetTree(data []Menu) []utils.TreeNode {
	items := make([]*Menu, 0, len(data))
	for i := range data {
		items = append(items, &data[i])
	}
	return utils.BuildTree[*Menu](items)
}
